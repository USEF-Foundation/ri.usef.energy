...
import energy.usef.brp.workflow.settlement.initiate.RequestPenaltyDataParameter;
...
/**
 * Workflow step implementation for the Request penalty data of the 'BRP Initiate Settlement' workflow. This implementation expects
 * to find the following parameters as input:
 * <ul>
 * <li>SETTLEMENT_DTO: the complete settlement ({@link SettlementDto})</li>
 * <li>PTU_DURATION: duration of one ptu in minutes ({@link Integer})</li>
 * </ul>
 * This implementation must return the following parameters as input:
 * <ul>
 * <li>UPDATED_SETTLEMENT_DTO: the settlement enriched with penalty data for each flex order, for each ptu</li>
 * </ul>
 */
public class BrpRequestPenaltyDataStub implements WorkflowStep {
    private static final Logger LOGGER = LoggerFactory.getLogger(BrpRequestPenaltyDataStub.class);
    public static final int MINUTES_PER_HOUR = 60;
    public static final int HOURS_PER_DAY = 24;
    public static final int MAXIMUM_DAYS_PER_MONTH = 31;
    public static final int FACTOR_MEGA = 1000000;
    ...
    public WorkflowContext invoke(WorkflowContext context) {
        ...
        Map<Integer, BigDecimal> apxPricesMap = pbcFeederService
                .retrieveApxPrices(firstDayOfMonth, 1, (HOURS_PER_DAY * MINUTES_PER_HOUR / ptuDuration) * MAXIMUM_DAYS_PER_MONTH);
        ...
        for (FlexOrderSettlementDto flexOrderSettlementDto : settlementDto.getFlexOrderSettlementDtos()) {
            for (PtuSettlementDto ptuSettlementDto : flexOrderSettlementDto.getPtuSettlementDtos()) {
                ...
                Integer startIndex = PtuUtil.numberOfPtusBetween(firstDayOfMonth, flexOrderSettlementDto.getPeriod(), 1,
    ...
    private BigDecimal calculatePenalty(BigDecimal apxPrice, int ptuDuration, BigInteger powerDeficiency) {
        // APX price is price per MWh, PowerdDeficiency is Watt during one PTU, so we must convert:
        // penalty = (apxPrice / (1000000 * (60 / <ptuDuration>))) * <powerDeficiency>
        return apxPrice.divide(BigDecimal.valueOf(FACTOR_MEGA * (MINUTES_PER_HOUR / ptuDuration)))
