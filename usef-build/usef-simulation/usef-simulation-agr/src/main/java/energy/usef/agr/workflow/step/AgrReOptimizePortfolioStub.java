 ...
 * This implementation only reduce the forecast consumption of the udis. It will not do anything with production of power and it
 * will not increase the forecast of udis.
 * <p>
...
public class AgrReOptimizePortfolioStub implements WorkflowStep {
    ...
    public WorkflowContext invoke(WorkflowContext context) {
        ...
        calculatePortfolioForecast(connectionPortfolioPerConnectionGroup, flexFactorPerPtuPerConnectionGroup,
                sumPotentialFlexPerPtuPerConnectionGroup, ptuDate, currentPtuIndex, ptuDuration);
    ...
    private void distributeTargetPower(Map<String, List<UdiEventDto>> udiEventsPerUdi, LocalDate ptuDate, int ptuDuration,
        ...
        BigInteger toBeDistributed = new BigDecimal(targetPower).multiply(DEVICE_MESSAGE_TARGET_FACTOR)
                .toBigInteger();
        ...
        for (UdiPortfolioDto udi : udis) {
            ...
            toBeDistributed = createReduceRequestsForUdi(udi, udiEvents,
                    deviceMessageDtoList, toBeDistributed, ptuDate, ptuIndex, ptuDuration);
        ...
        if (toBeDistributed.compareTo(BigInteger.ZERO) < 0) {
            LOGGER.warn("There is still some power ({}) left after trying to distribute the target power {} "
                    + "amongst all udi's with ReduceCapabilities of type CONSUMPTION for ptu "
                    + "index {}.", toBeDistributed, targetPower, ptuIndex);
        }
    }
    ...
    private BigInteger createReduceRequestsForUdi(UdiPortfolioDto udi, List<UdiEventDto> udiEvents,
        ...
        BigInteger allocatedFlex = udi.getUdiPowerPerDTU().get(startDtu).getForecast().getAllocatedFlexConsumption();
        ...
        for (UdiEventDto udiEventDto : filteredUdiEvents) {
            ...
            // get the first reduce consumption capability
            Optional<DeviceCapabilityDto> deviceCapability = udiEventDto.getDeviceCapabilities()
                    ...
                    .filter(deviceCapabilityDto -> ((ReduceCapabilityDto) deviceCapabilityDto).getConsumptionProductionType()
                            == ConsumptionProductionTypeDto.CONSUMPTION)
                    .findFirst();
            ...
            if (deviceCapability.isPresent()) {
                ...
                if (reducePower.compareTo(BigInteger.ZERO) < 0) {
                    ...
                    deviceMessageDto.getReduceRequestDtos().add(
                            createReduceRequest(ptuDate, dtusPerPtu, startDtu, udiEventDto, reducePower));
                    deviceMessageDtoList.add(deviceMessageDto);

                    updateAllocatedFlex(udi, dtusPerPtu, startDtu, allocatedFlex, reducePower);
                }
                // keep track of all the amount of power that already is reduced
                toBeDistributed = toBeDistributed.subtract(reducePower);
                if (toBeDistributed.compareTo(BigInteger.ZERO) >= 0) {
                    break;
                }
            }
    ...
    private BigInteger calculateReducePower(BigInteger toBeDistributed, BigInteger allocatedFlex, ReduceCapabilityDto capability) {
        ...
        // use the 'less negative' for further calculation: we assume that both numbers are negative (basic implementation)
        maxReduce = (toBeDistributed.compareTo(maxReduce) > 0) ? toBeDistributed : maxReduce;
    ...
    private void updateAllocatedFlex(UdiPortfolioDto udi, int dtusPerPtu, int startDtu, BigInteger allocatedFlex,
    ...
    private ReduceRequestDto createReduceRequest(LocalDate ptuDate, int dtusPerPtu, int startDtu,
            UdiEventDto udiEventDto, BigInteger reducePower) {
    ...
    private void calculatePortfolioForecast(Map<String, List<ConnectionPortfolioDto>> connectionPortfolioPerConnectionGroup,
            Map<String, Map<Integer, BigDecimal>> potentialFlexFactorPerPtuPerConnectionGroup,
            Map<String, Map<Integer, BigInteger>> summedPotentialFlexConsumptionPerPtuPerConnectionGroup, LocalDate ptuDate,
            int currentPtuIndex, int ptuDuration) {
        final LocalDate currentDate = DateTimeUtil.getCurrentDate();
        connectionPortfolioPerConnectionGroup.forEach(
                (connectionGroupId, connectionPortfolioList) -> summedPotentialFlexConsumptionPerPtuPerConnectionGroup.get(
                        connectionGroupId).forEach((ptuIndex, summedPotentialFlex) -> {
    ...
    private void divideFlexOverUdis(ConnectionPortfolioDto connectionPortfolioDTO, BigDecimal factor, int ptuIndex,
        ...
        for (UdiPortfolioDto udi : connectionPortfolioDTO.getUdis()) {
            ...
            for (int dtuIndex = startDtu; dtuIndex < startDtu + dtusPerPtu; dtuIndex++) {
                ...
                // CAREFUL: this is naive implementation, for now we don't support the increase in CONSUMPTION (positive factored
                // flex)
