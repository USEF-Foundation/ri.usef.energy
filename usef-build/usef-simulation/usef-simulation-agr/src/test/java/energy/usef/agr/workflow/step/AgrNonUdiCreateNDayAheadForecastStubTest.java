...
/**
 * Unit test to test the AgrNonUdiCreateNDayAheadForecastStub.
 */
@RunWith(PowerMockRunner.class)
public class AgrNonUdiCreateNDayAheadForecastStubTest {
    private AgrNonUdiCreateNDayAheadForecastStub agrNonUdiCreateNDayAheadForecastStub;
    ...
    @Before
    public void init() {
        agrNonUdiCreateNDayAheadForecastStub = new AgrNonUdiCreateNDayAheadForecastStub();
        Whitebox.setInternalState(agrNonUdiCreateNDayAheadForecastStub, "pbcFeederService", pbcFeederService);
    }
    ...
    public void testInvoke() {
        ...
        Map<String, List<ConnectionPortfolioDto>> connectionDtosPerCongestionPoint = new HashMap<>();
        connectionDtosPerCongestionPoint.put("ean.1234567890", connections);

        Mockito.when(pbcFeederService
                .updateConnectionUncontrolledLoadForecast(Matchers.any(LocalDate.class), Matchers.anyInt(), Matchers.anyInt(), Matchers.any()))
                .thenReturn(connections);

        Mockito.when(pbcFeederService
                .retrieveNonUDIListWithForecast(Matchers.any(LocalDate.class), Matchers.any(),
                        Matchers.anyInt())).thenReturn(connections);

        workflowContext = agrNonUdiCreateNDayAheadForecastStub.invoke(workflowContext);
