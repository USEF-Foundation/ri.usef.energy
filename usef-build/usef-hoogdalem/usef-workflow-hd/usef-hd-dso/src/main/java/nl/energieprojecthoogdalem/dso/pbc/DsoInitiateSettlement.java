...
import info.usef.dso.workflow.DsoWorkflowStep;
...
/**
 * Implementation of the Dso Initiate Settlement workflow.
 * <p>
 ...
 * </ul>
 */
public class DsoInitiateSettlement implements WorkflowStep {

    private static final Logger LOGGER = LoggerFactory.getLogger(DsoInitiateSettlement.class);
    ...
    private BigInteger fetchActualPower(Map<String, Map<LocalDate, List<PtuMeterDataDto>>> ptuMeterData,
        ...
        if (ptuMeterData != null) {
            PtuMeterDataDto smartMeterValue = ptuMeterData.getOrDefault(participantDomain, new HashMap<>()).get(period).stream()
                    ...
                    .findFirst().orElse(noSmartMeterValue(index));
    ...
    private PtuMeterDataDto noSmartMeterValue(int index) {
        PtuMeterDataDto smartMeterValue = new PtuMeterDataDto(index);
        smartMeterValue.setPower(BigInteger.ZERO);
        return smartMeterValue;
    }
