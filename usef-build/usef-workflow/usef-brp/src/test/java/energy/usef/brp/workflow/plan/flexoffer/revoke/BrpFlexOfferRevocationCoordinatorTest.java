...
/**
 * Test class in charge of the unit tests related to the {@link BrpFlexOfferRevocationCoordinator}.
 */
@RunWith(PowerMockRunner.class)
public class BrpFlexOfferRevocationCoordinatorTest {

    private static final Logger LOGGER = LoggerFactory.getLogger(BrpFlexOfferRevocationCoordinatorTest.class);

    private BrpFlexOfferRevocationCoordinator coordinator;
    ...
    @Before
    public void init() {
        coordinator = new BrpFlexOfferRevocationCoordinator();
    ...
    private FlexOfferRevocation buildContext() {
        ...
        flexOfferRevocation.setMessageMetadata(new MessageMetadataBuilder().messageID().conversationID().timeStamp()
                ...
                .recipientDomain("recipient-usef-example.com").recipientRole(USEFRole.BRP).build());
        return flexOfferRevocation;
    }

    private Map<Integer, PtuFlexOffer> buildFlexOffers() {
        PtuFlexOffer fo1 = new PtuFlexOffer();
        PtuContainer ptuc1 = new PtuContainer();
        ptuc1.setPtuIndex(1);
        fo1.setPtuContainer(ptuc1);
        return Collections.singletonList(fo1).stream().collect(Collectors.toMap(fo -> fo.getPtuContainer().getPtuIndex(),
                Function.identity()));
    }
