...
/**
 * Abstract class for {@link DeviceRequest}, with the core information
 */
@Entity
...
public abstract class DeviceRequest {
    ...
    private Date period;
    @ManyToOne
    @JoinColumn(name = "DEVICE_MESSAGE_ID", foreignKey = @ForeignKey(name = "DR_DEVICE_MESSAGE_FK"), nullable = false)
    private DeviceMessage deviceMessage;

    public DeviceRequest(LocalDate period) {
        setPeriod(period);
    }
