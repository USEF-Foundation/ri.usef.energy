 ...
 */
@Stateless
public class DsoCommonReferenceUpdateResponseCoordinator {

    private static final Logger LOGGER = LoggerFactory.getLogger(DsoCommonReferenceUpdateResponseCoordinator.class);
    ...
    public void handleEvent(@Observes CommonReferenceUpdateResponseEvent event) {
        ...
        if (DispositionAcceptedRejected.ACCEPTED.equals(event.getCommonReferenceUpdateResponse().getResult())) {
            businessService.updateCongestionPointStatusForCRO(event.getCommonReferenceUpdateResponse().getEntityAddress(), event
                    .getCommonReferenceUpdateResponse().getMessageMetadata()
                    .getSenderDomain());
