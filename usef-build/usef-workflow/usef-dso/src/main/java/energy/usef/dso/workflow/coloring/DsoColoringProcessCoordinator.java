...
public class DsoColoringProcessCoordinator {
     ...
     */
    @Asynchronous
    public void handleEvent(@Observes(during = TransactionPhase.AFTER_COMPLETION) ColoringProcessEvent event) {
        ...
        List<GridSafetyAnalysis> gridSafetyAnalysisList = dsoPlanboardBusinessService
                ...
                event.getCongestionPoint(), event.getDate());
        if (gridSafetyAnalysisList.isEmpty()) {
            LOGGER.info("No grid safety analysis found for date {} and congestion point {}. No coloring needed.", event.getDate(),
        ...
        invokePostColoringProcessPBC(event.getDate(), orangePtuContainerDtoList);
