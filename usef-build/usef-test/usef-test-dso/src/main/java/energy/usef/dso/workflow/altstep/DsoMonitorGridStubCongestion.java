...
public class DsoMonitorGridStubCongestion implements WorkflowStep {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(DsoMonitorGridStubCongestion.class);
    ...
    public WorkflowContext invoke(WorkflowContext context) {
        ...
        if (totalLoad.compareTo(new BigDecimal(0)) < 0) {
            BigDecimal upperLimit = powerLimits.get(1).add(new BigDecimal(-1));
            LOGGER.debug("Total load: [{}], powerlimits: [{}],[{}]", totalLoad, powerLimits.get(0), upperLimit);
            context.setValue(DsoMonitorGridStepParameter.OUT.MIN_LOAD.name(), powerLimits.get(0).longValue());
            context.setValue(DsoMonitorGridStepParameter.OUT.MAX_LOAD.name(), upperLimit.longValue());
        } else {
            BigDecimal lowerLimit = powerLimits.get(1).add(new BigDecimal(-1));
            LOGGER.debug("Total load: [{}], powerlimits: [{}],[{}]", totalLoad, lowerLimit, powerLimits.get(1));
            context.setValue(DsoMonitorGridStepParameter.OUT.MIN_LOAD.name(), lowerLimit.longValue());
            context.setValue(DsoMonitorGridStepParameter.OUT.MAX_LOAD.name(), powerLimits.get(1).longValue());
        }
        LOGGER.info("Congestion enforced by alternative stub");
