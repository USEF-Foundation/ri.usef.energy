 ...
 * This stub does orders all offers.
 *
 ...
 * </ul>
 */
public class DsoPlaceFlexOrdersStubOrder implements WorkflowStep {
    ...
    public WorkflowContext invoke(WorkflowContext context) {
        ...
        offers.stream().filter(flexOffer -> !checkIfFlexOfferIsEmpty(flexOffer)).
     ...
     */
    private boolean checkIfFlexOfferIsEmpty(FlexOfferDto flexOffer) {
        return flexOffer.getPtus().stream().filter(flexOfferPtu -> flexOfferPtu.getPower().compareTo(BigInteger.ZERO) == 0).count()
                == 96;
    }
}
