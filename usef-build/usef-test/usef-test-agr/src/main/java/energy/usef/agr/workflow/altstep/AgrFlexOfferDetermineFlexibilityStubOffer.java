...
public class AgrFlexOfferDetermineFlexibilityStubOffer implements WorkflowStep {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AgrFlexOfferDetermineFlexibilityStubOffer.class);
    ...
    private Map<Integer, Map<ConsumptionProductionTypeDto, BigInteger>> fetchPotentialFlexForConnectionList(
        ...
        connectionPortfolioDtos.stream()
                ...
                .map(udi -> PowerContainerDtoUtil.average(udi, period, ptuDuration)) // average power containers per udi
    ...
    private void buildPotentialFlexMapForPtu(Map<Integer, Map<ConsumptionProductionTypeDto, BigInteger>> potentialFlexPerPtu,
        ...
        } else if (potentialFlexConsumption.signum() > -1) { // positive or ZERO: add to CONSUMPTION (== consumes more)
            powerMap.put(
                    ConsumptionProductionTypeDto.CONSUMPTION,
                    powerMap.getOrDefault(ConsumptionProductionTypeDto.CONSUMPTION, ZERO).add(potentialFlexConsumption));
        } else { // negative: add to PRODUCTION (== consumes less)
            powerMap.put(
                    ConsumptionProductionTypeDto.PRODUCTION,
                    powerMap.getOrDefault(ConsumptionProductionTypeDto.PRODUCTION, ZERO).add(potentialFlexConsumption.abs()));
        }
        ...
        } else if (potentialFlexProduction.signum() > -1) { // positive or ZERO: add to PRODUCTION (== produces more)
            powerMap.put(
                    ConsumptionProductionTypeDto.PRODUCTION,
                    powerMap.getOrDefault(ConsumptionProductionTypeDto.PRODUCTION, ZERO).add(potentialFlexProduction));
        } else { // negative: add to CONSUMPTION (== produces less)
            powerMap.put(
                    ConsumptionProductionTypeDto.CONSUMPTION,
                    powerMap.getOrDefault(ConsumptionProductionTypeDto.CONSUMPTION, ZERO).add(potentialFlexProduction.abs()));
        }
    }
    ...
    private Map<String, Map<Integer, Map<ConsumptionProductionTypeDto, BigInteger>>> fetchAlreadyOfferedFlex(
        ...
        BiFunction<Map<ConsumptionProductionTypeDto, BigInteger>, Map<ConsumptionProductionTypeDto, BigInteger>,
            ...
            Map<ConsumptionProductionTypeDto, BigInteger> result = new HashMap<>();
            result.put(ConsumptionProductionTypeDto.CONSUMPTION,
                    map1.getOrDefault(ConsumptionProductionTypeDto.CONSUMPTION, ZERO).add(map2.getOrDefault(
                            ConsumptionProductionTypeDto.CONSUMPTION, ZERO)));
            result.put(ConsumptionProductionTypeDto.PRODUCTION,
                    map1.getOrDefault(ConsumptionProductionTypeDto.PRODUCTION, ZERO).add(map2.getOrDefault(
