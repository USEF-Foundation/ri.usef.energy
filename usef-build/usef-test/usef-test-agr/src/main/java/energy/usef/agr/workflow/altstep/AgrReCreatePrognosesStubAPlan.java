 ...
 * re-optimization of the portfolio. This version only recreates the A-Plans.
 * <p>
 ...
 * </ul>
 */
public class AgrReCreatePrognosesStubAPlan implements WorkflowStep {

    private static final Logger LOGGER = LoggerFactory.getLogger(AgrReCreatePrognosesStubAPlan.class);
    ...
    private List<Long> compareAPlansAgainstLoad(List<PrognosisDto> aPlans,
            Map<String, Map<Integer, BigInteger>> loadPerPtuPerConnectionGroup) {
        List<Long> toBeRecreatedSequences = new ArrayList<>();
        aPlans.stream().forEach(aplan -> aplan.getPtus().stream()
