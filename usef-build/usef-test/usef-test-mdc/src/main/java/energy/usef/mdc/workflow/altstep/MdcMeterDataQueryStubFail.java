 ...
 */
public class MdcMeterDataQueryStubFail implements WorkflowStep {
    private static final Logger LOGGER = LoggerFactory.getLogger(MdcMeterDataQueryStubFail.class);
        ...
        IntStream.rangeClosed(1, ptuCount).forEach(index -> {
            ...
            if (ptuToUncontrolledLoad.get(index) == null) {
                ...
                BigInteger randomizedUncontrolledLoad = ptuToUncontrolledLoad.get(index)
                        .getPower()
                        .add(BigInteger.valueOf(generateRandomNumber(-50, 50)));
